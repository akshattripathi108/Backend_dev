/**
 * Test Suite for Session & Cookie Authentication
 * Tests login, session validation, profile retrieval, and logout
 */
  ...
  try {
    ...
    try {
      ...
      if (res.status === 200 && res.body.success) {
        ...
        if (setCookieHeaders) {
          ...
          console.log(`   Session cookie set: ${sessionCookie.substring(0, 30)}...`);
        }
        testsPassed++;
      } else {
    ...
    try {
      ...
      if (res.status === 200 && res.body.isActive) {
        ...
        throw new Error('Session check failed - no active session');
      }
    } catch (err) {
    ...
    try {
      ...
      console.log('❌ Session status test failed:', err.message);
      testsFailed++;
    }
    ...
    try {
      ...
      console.log('❌ Invalid login test failed:', err.message);
      testsFailed++;
    }
    ...
    try {
      ...
      if (res.status === 401) {
        ...
        throw new Error('Should not be able to access protected route');
      }
    } catch (err) {
      console.log('❌ Post-logout access test failed:', err.message);
      testsFailed++;
    }

    // Test 9: Get available routes
    ...
    try {
      ...
      console.log('❌ Routes test failed:', err.message);
      testsFailed++;
    }
